...
import { $hotelInfo,$inventory,$moneyReduce} from "../../api/userApi";
...
export default function HotelDetail() {
    ...
    const [orderInfo,setOrderInfo]=useState({EachPrice:null,inOutDay:['',''],roomName:''})
    const [selectRoom,setSelectRoom]=useState('all')
    const [notiMsg, setNotiMsg] = useState({ type: "", description: "" });
    const [isTop,setIsTop]=useState(false)
    useEffect(() => {
        console.log('user',user)
        let hotelId=params.get('id') //接收路由跳转传过来的酒店id
        async function getHotelRoom(){
            ...
            setRoom(data.room)
            setShowRoomList(data.room)
        }
        getHotelRoom()
    },[])  
    useEffect(()=>{
        const fixedTop = roomNav.current.offsetTop;
    ...
    function payInfo(price,roomtype,inventory){ //点击预定按钮，传递订单信息，并弹出支付界面
        if(inventory<=0){
            setNotiMsg({ type: "success", description: '已售罄，请选择别的房型' });
        }else if(liveDays==0){
            setNotiMsg({ type: "success", description: '请先选择入住日期' });
        }else{
    ...
    async function addOrder(state,statemsg){  
        const newOrder={uid:user.uid,oname:name,location:adress,starttime:inOutDate[0],endtime:inOutDate[1],price:liveDays*orderInfo.EachPrice,state:state,statemsg}
        const requestData=await $neworder(newOrder)
    }
    ...
    function submitOrder(){     
        ...
        async function moneyReduce(userId,price){ //给用户发请求，扣钱 
            const requestData =await $moneyReduce(userId,price) //参数:用户id，钱
            if(requestData.meta.msg=='对不起，您的钱包余额不足'){
                setNotiMsg({ type: "error", description: '您的余额不足' });
            }else{
                inventoryUpdate() //发送请求更新库存
                addOrder('togo','待出行')// 给订单那边发送请求，给这个用户新增一个成功的订单
                setNotiMsg({ type: "success", description: '支付成功' });
            }
        }
        moneyReduce(user.uid,liveDays*orderInfo.EachPrice)   
    }

    function cancelOrder(){  // 需提交请求，给这个账户增加一个待支付的订单  ---和订单那边联动
        setPayPageShow(false)
        addOrder('topay','待付款')
        setNotiMsg({ type: "error", description: '您的订单支付失败' });      
    }

    function showRoom(select,znSelect){
        setSelectRoom(select)
        let newRoom=[]
        if(select=='all'){
            setShowRoomList(room)
            return 
        }
        for(let i=0;i<room.length;i++){
            if(select!='rest'){
                if(room[i].label.includes(znSelect)){
                    newRoom.push(room[i])
                }
            }else{
                if(room[i].inventory!=0){
                    newRoom.push(room[i])
                }
            }
        }
        setShowRoomList(newRoom)
    }
    ...
    <div>
            ...
            <div className='detaiHeadline'>
                <div className='detaiHeadlineLeft'>
                    <div className='detailHeadlineTitle'>
                        {name}
                        {isRecommend&&<img className="recommendImg" loading='lazy' src={require('../../asstes/recommend.jpg')}  alt="糟糕！图片加载不出来啦" />}
                    </div>
                    <div className='detailHeadlineAddress'>
                        <img className="iconImg" loading='lazy' src={require('../../asstes/iconfont/location.png')}  alt="糟糕！图片加载不出来啦" />
                        {adress}
                    </div>
                    <div className='detailHeadlineUtility'>
                        <img className="iconImg" loading='lazy' src={require('../../asstes/iconfont/building.png')}  alt="糟糕！图片加载不出来啦" />
        ...
        <section className='hotelDetailRoom'>
            <div className={['hotelDetailNav',isTop?'fixedTop':null].join(' ')} ref={roomNav}>
            ...
            <div className='hotelRoomList' ref={search}>
                <div className='search'>
                    <RangePicker className='datepick' disabledDate={disabledDate} format="YYYY-MM-DD" onChange={onChange} placeholder={['开始日期','结束日期']}/>
                    <span>{liveDays}晚</span>
                </div>
                <div className='filter'>
                    <div className={['filterItem',selectRoom=='all'?'active':null].join(' ')} onClick={()=>{showRoom('all','全部')}}>全部</div>
                    <div className={['filterItem',selectRoom=='bigBed'?'active':null].join(' ')} onClick={()=>{showRoom('bigBed','大床房')}}>大床房</div>
                    <div className={['filterItem',selectRoom=='twobed'?'active':null].join(' ')} onClick={()=>{showRoom('twobed','双床房')}}>双床房</div>
                    <div className={['filterItem',selectRoom=='cancelForfree'?'active':null].join(' ')} onClick={()=>{showRoom('cancelForfree','免费取消')}}>免费取消</div>
                    <div className={['filterItem',selectRoom=='rest'?'active':null].join(' ')} onClick={()=>{showRoom('rest','可订')}}>可订</div>
                </div>
                            ...
                            return  <div className={['roomItem',item.inventory==0?'deactive':null].join(' ')} key={index}>
                                        <div className='roomItemLeft'>
                                            <div className='roomPic'>
                                                <img className="roomImg" loading='lazy' src={item.img}  alt="糟糕！图片加载不出来啦" />
                                            </div>
        ...
        {payPageShow&&<section className='hotelPay'>
